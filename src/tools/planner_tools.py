...
def _handle_log_completion(args: dict) -> dict:
    ...
    if mgr:
        ...
        if estimate and actual_hours:
            ...
            if ratio > 1.5:
                insight = (
                    f"Student underestimated {subject} task '{title}': estimated {estimate}h, took {actual_hours}h"
                )
            elif ratio < 0.6:
                insight = f"Student overestimated {subject} task '{title}': estimated {estimate}h, took {actual_hours}h"
            else:
                insight = (
                    f"Student completed {subject} task '{title}' on track: estimated {estimate}h, took {actual_hours}h"
                )
        else:
...
register(
    ToolDefinition(
        ...
        description=(
            "Update the status of a calendar task (assignment or exam)."
            " Use when a student reports progress on a task."
        ),
        parameters={
            "type": "object",
            "properties": {
...
register(
    ToolDefinition(
        ...
        description=(
            "Log that a student has completed a task/assignment."
            " Records actual time spent and learns from the data for future predictions."
        ),
        parameters={
            "type": "object",
            "properties": {
...
register(
    ToolDefinition(
        ...
        description=(
            "Get a prioritized list of upcoming assignments and exams with smart scoring"
            " based on deadlines, estimated effort, and student's historical performance patterns."
        ),
        parameters={
            "type": "object",
            "properties": {
...
register(
    ToolDefinition(
        ...
        description=(
            "Get the student's performance patterns and competency data for a specific subject or course."
            " Shows historical completion times and behavioral insights."
        ),
        parameters={
            "type": "object",
            "properties": {
                "subject": {
                    "type": "string",
                    "description": "The subject or course name to check (e.g. 'Math', 'Physics', 'Writing')",
                },
            },
            "required": ["subject"],
        },
